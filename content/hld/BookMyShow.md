...
4. **Redis Cache** — Caches show listings and aggregated availability.
...
2. **Lock Manager** — Acquires distributed locks on seats with TTL. The critical path for preventing double-booking.
...
> 💡 *SET NX = "Set only if Not eXists" — an atomic compare-and-swap in one round trip. Combined with EX (expire), it gives us a lock that auto-releases.*
