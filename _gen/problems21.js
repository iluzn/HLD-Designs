...
MORE21.push({ slug: 'minimized-maximum-of-products-distributed-to-any-store', title: 'Minimized Maximum of Products Distributed to Any Store', difficulty: 'medium', topics: ['Array', 'Binary Search'], type: 'ARR_TGT_INT', langsrc: T.ARR_TGT_INT('minimizedMaximum'),
  desc: '<p>You have <code>n</code> product types. The <code>i</code>th type has <code>nums[i]</code> units in stock. You want to distribute <strong>all</strong> units across <code>target</code> specialty stores with these rules:</p><ul><li>Each store carries <strong>at most one</strong> product type (but as many units of that type as you assign).</li><li>Some stores may remain empty.</li></ul><p>Minimize the <strong>maximum</strong> number of units assigned to any single store. Return that minimized maximum.</p>',
  examples: [{ in: 'nums = [11,6], target = 10', out: '2', ex: 'Assign product 0 (11 units) across 6 stores: ceil(11/2)=6 stores each getting at most 2. Assign product 1 (6 units) across 3 stores: ceil(6/2)=3 stores each getting at most 2. Uses 9 stores total (<=10). The max any store gets is 2.' }, { in: 'nums = [15,10,10], target = 7', out: '5', ex: 'With max=5: product 0 needs ceil(15/5)=3, product 1 needs ceil(10/5)=2, product 2 needs ceil(10/5)=2. Total=7 stores. With max=4: 4+3+3=10>7. So 5 is the answer.' }],
