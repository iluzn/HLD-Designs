...
MORE15.push({ slug: 'min-stack', title: 'Min Stack', difficulty: 'medium', topics: ['Stack', 'Design'], type: 'OPS',
  ...
  editorial: ED({ intuition: 'The only hard part is O(1) <code>getMin</code>. Track the minimum <em>as of</em> each state on an auxiliary stack so the current minimum is always on top. Every operation is already O(1) — that is optimal in time; the refinement below trims memory by storing each running minimum <em>once</em> with a repeat count instead of duplicating a min for every element.', approach: ['Keep the value stack plus a mins stack of <code>(min, count)</code> pairs.', 'On push: if val is a new minimum push <code>(val, 1)</code>; if it ties the current min bump its count; otherwise leave mins untouched.', 'On pop: if the popped value equals the current min, decrement its count and drop the pair when it reaches zero.', 'top reads the value stack; getMin reads the top min value. All four operations stay O(1).'], code: 'class MinStack:\n    def __init__(self):\n        self.stack = []\n        self.mins = []              # (min_value, count)\n    def push(self, val):\n        self.stack.append(val)\n        if not self.mins or val < self.mins[-1][0]:\n            self.mins.append([val, 1])\n        elif val == self.mins[-1][0]:\n            self.mins[-1][1] += 1\n    def pop(self):\n        val = self.stack.pop()\n        if val == self.mins[-1][0]:\n            self.mins[-1][1] -= 1\n            if self.mins[-1][1] == 0:\n                self.mins.pop()\n    def top(self):\n        return self.stack[-1]\n    def getMin(self):\n        return self.mins[-1][0]', time: 'O(1)', timeWhy: 'push, pop, top, and getMin are all constant time — optimal.', space: 'O(k)', spaceWhy: 'the mins stack holds at most one entry per distinct running minimum (k \u2264 n), fewer than one min per element.', pitfalls: ['Compare with <code>&lt;</code> on push and <code>==</code> on pop so repeated minimums are counted and released correctly.', 'A plain second stack that mirrors every push also works and is O(1); the count just saves memory.'] }),
  custom: {
