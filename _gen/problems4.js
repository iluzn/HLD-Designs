...
MORE4.push({ slug: 'count-complete-tree-nodes', title: 'Count Complete Tree Nodes', difficulty: 'easy', topics: ['Tree', 'DFS', 'Binary Search'], type: 'TREE_INT', langsrc: T.TREE_INT('countNodes'),
  ...
  editorial: ED({ intuition: 'A plain traversal counts every node in O(n), but a <em>complete</em> tree lets us do better. Walk the far-left spine and the far-right spine: if their heights match, this subtree is <em>perfect</em> and holds exactly <code>2^h - 1</code> nodes — no need to visit any of them. Otherwise recurse into both children. Only the subtrees straddling the last, partially-filled level get expanded.', approach: ['Measure the left-spine height <code>lh</code> (keep going left) and right-spine height <code>rh</code> (keep going right).', 'If <code>lh == rh</code> the subtree is perfect: return <code>(1 &lt;&lt; lh) - 1</code> in O(1).', 'Otherwise return <code>1 + countNodes(left) + countNodes(right)</code>.', 'Each level triggers at most one non-perfect recursion, and each height probe is O(log n).'], code: 'def countNodes(self, root):\n    if not root:\n        return 0\n    lh = rh = 0\n    n = root\n    while n:\n        lh += 1; n = n.left\n    n = root\n    while n:\n        rh += 1; n = n.right\n    if lh == rh:                 # perfect subtree\n        return (1 << lh) - 1\n    return 1 + self.countNodes(root.left) + self.countNodes(root.right)', time: 'O(log\u00b2 n)', timeWhy: 'O(log n) recursion depth, each doing an O(log n) height probe; perfect subtrees are counted in O(1).', space: 'O(log n)', spaceWhy: 'recursion stack on a balanced complete tree.' }),
