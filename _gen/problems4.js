...
MORE4.push({ slug: 'count-complete-tree-nodes', title: 'Count Complete Tree Nodes', difficulty: 'easy', topics: ['Tree', 'DFS', 'Binary Search'], type: 'TREE_INT', langsrc: T.TREE_INT('countNodes'),
  ...
  editorial: ED({ intuition: 'Count every node with a simple traversal: total = 1 + nodes in the left subtree + nodes in the right subtree. (For a guaranteed <em>complete</em> tree there is an O(log\u00b2 n) left/right-spine trick, but the straightforward O(n) count is clear and always correct.)', approach: ['Null \u2192 0.', 'Otherwise <code>1 + countNodes(left) + countNodes(right)</code>.'], code: 'def countNodes(self, root):\n    if not root:\n        return 0\n    return 1 + self.countNodes(root.left) + self.countNodes(root.right)', time: 'O(n)', timeWhy: 'visits every node once.', space: 'O(h)', spaceWhy: 'recursion stack, height h.' }),
